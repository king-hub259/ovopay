...
class _GetOfferWidgetState extends State<GetOfferWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    final creatorImageSet =
        "${ApiEndpoint.mainDomain}/${widget.imagePath}/${widget.getOffer.creatorimage}";
    final defaultImagePath = "${ApiEndpoint.mainDomain}/${widget.defaultImage}";

    final creatorImage =
        widget.getOffer.creatorimage != '' ? creatorImageSet : defaultImagePath;
                    ...
                    children: [
                      ...
                      if (widget.getOffer.tradestatus == 1) ...[
                        if (widget.getOffer.receiverid.toString() == LocalStorages.getId() ||
                            widget.getOffer.receiverid.toString() != LocalStorages.getId()) ...[
                          if (widget.getOffer.status == 4) ...[
                            ...
                            if (widget.getOffer.tradeuserid.toString() == LocalStorages.getId()) ...[
                              _customButtonWidget(
  ...
  void _onCounterOfferProcessMethod() {
    ...
    counterOfferController.rateCurrency.value = widget.getOffer.ratecurrencycode!;
    counterOfferController.rateAmount.value = double.parse(widget.getOffer.traderate!);
    counterOfferController.rateController.text = widget.getOffer.traderate!;

    /// >>> get sell amount and currency from get offer list
    counterOfferController.sellAmount.value = double.parse(widget.getOffer.tradeamount!);
    counterOfferController.sellCurrency.value = widget.getOffer.salecurrencycode!;
    ...
    counterOfferController.counterSellAmount.value = double.parse(widget.getOffer.amount!);
    counterOfferController.counterRateAmount.value = double.parse(widget.getOffer.rate!);
    ...
    counterOfferController.receiverId.value = widget.getOffer.receiverid.toString();

    /// >>> get creator id for counter offer [-- Required --]
    counterOfferController.creatorId.value = widget.getOffer.creatorid.toString();
